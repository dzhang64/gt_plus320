...
import com.gt_plus.modules.act.dao.ActDao;
import com.gt_plus.modules.act.entity.Act;
import com.gt_plus.modules.act.utils.ActUtils;
import com.gt_plus.modules.act.utils.ProcessDefCache;
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	 ...
	 * @return
	 */
	public List<Act> toClaimList(Act act){
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
...
//			e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
	...
	public Page<Act> sendList(Page<Act> page, Act act){
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		
		HistoricProcessInstanceQuery hisProcInsQuery = historyService
				.createHistoricProcessInstanceQuery().startedBy(userId)
				.includeProcessVariables().orderByProcessInstanceStartTime()
				.desc();
		// 查询总数
		page.setCount(hisProcInsQuery.count());
		page.initialize();
		
		// 查询列表
		List<HistoricProcessInstance> listPage = hisProcInsQuery.listPage(page.getFirstResult(), page.getMaxResults());
		List<String> processInstanceIds = Lists.newArrayList();
		for (HistoricProcessInstance hisProcIns : listPage) {
			processInstanceIds.add(hisProcIns.getId());
			hisProcIns.getProcessDefinitionId();
		}
		if (processInstanceIds.size() == 0) {
			return null;
		}
		HistoricTaskInstanceQuery histTaskQuery = historyService.createHistoricTaskInstanceQuery().processInstanceIdIn(processInstanceIds)
...
//					e.setProcExecUrl(ActUtils.getProcExeUrl(task.getProcessDefinitionId()));
			e.setStatus("finish");
			//temp.put(histTask.getProcessInstanceId(), e);
			page.getList().add(e);
		}
		/*for (Entry<String, Object> entry : temp.entrySet()) {
			Act a = (Act)entry.getValue();
			page.getList().add(a);
		}
		page.setCount(temp.size());*/
		act.setPage(page);
		return page;
//		HistoricTaskInstanceQuery histTaskQuery = historyService.createHistoricTaskInstanceQuery().taskAssignee(userId).finished()
//				.includeProcessVariables().orderByHistoricTaskInstanceEndTime().desc();
		...
		// 设置查询条件
		if (StringUtils.isNotBlank(act.getProcDefKey())){
			hisProcInsQuery.processDefinitionKey(act.getProcDefKey());
		}
		...
		for (HistoricProcessInstance hisProcIns : listPage) {
			Act e = new Act();
			e.setHisProcIns(hisProcIns);
			e.setVars(hisProcIns.getProcessVariables());
			e.setProcDef(ProcessDefCache.get(hisProcIns.getProcessDefinitionId()));
			e.setStatus("finish");
			page.getList().add(e);
		}
		act.setPage(page);
...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
			...
			else if("endEvent".equals(histIns.getActivityType()))
			{
				...
				// 获取任务执行人名称
			else if (StringUtils.isNotEmpty(histIns.getAssignee()))
			{
				...
				//下一步任务的执行人名称查询
			else if (StringUtils.isEmpty(histIns.getAssignee()))
			{		
					String exId= histIns.getExecutionId();
					String activitiId = histIns.getActivityId();
					//在这里需要默认用户活动ID和该用户活动对应的候选人ID相同
				    List<String> uli =  (List<String>) runtimeService.getVariable(exId,activitiId);
					User user = UserUtils.getByLoginName(uli.get(0));
					if (user != null)
					{
			...
			{
				boolean bl = false;
				Integer actNum1 = actMap.get(histIns.getActivityId());
				// 该活动节点，后续节点是否在结束节点之前，在后续节点中是否存在
				for (int j=i+1; j<list.size(); j++){
					HistoricActivityInstance hi = list.get(j);
					Integer actNumA = actMap.get(hi.getActivityId());
					if ((actNumA != null && actNumA < actNum1) || StringUtils.equals(hi.getActivityId(), histIns.getActivityId())){
						bl = true;
					}
				}
				if (!bl){
