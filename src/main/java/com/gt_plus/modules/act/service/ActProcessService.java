...
public class ActProcessService extends BaseService {
	 ...
	 * @return
	 */
	public List<ProcessDefinition> processDefinitionList(String category) {
		ProcessDefinitionQuery processDefinitionQuery = repositoryService.createProcessDefinitionQuery()
				.latestVersion().orderByProcessDefinitionKey().asc();
		if (StringUtils.isNotEmpty(category)){
		    processDefinitionQuery.processDefinitionCategory(category);
...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
	 ...
	 * 
	 * */
	public String findForm(String processId) {

	    String xmlForm=formService.getStartFormKey(processId);
		return xmlForm;
	}
