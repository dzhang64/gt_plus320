...
<%@ attribute name="queryName" type="java.lang.String" required="false" description="数据过滤表单对象对应的查询字段名称"%>
<%@ attribute name="queryId" type="java.lang.String" required="false" description="数据过滤表单对象ID"%>
<%@ attribute name="checkName" type="java.lang.String" required="false" description="传递选中的Name"%>
<script type="text/javascript">
function searchGrid${id}(){
	var queryValue = ''; 
	if ($("#${queryId}")) queryValue = $("#${queryId}").val();
	if ($("#${checkName}")) checkName = $("#${checkName}").val();
	top.layer.open({
	    type: 2,  
	    area: ['800px', '500px'],
	    title:"${title}",
	    name:'friend',
	    content: "${url}?fieldLabels=${fieldLabels}&fieldKeys=${fieldKeys}&url=${url}&searchLabel=${searchLabel}&searchKey=${searchKey}&queryName=${queryName}&queryValue="+queryValue+"&${queryName}="+queryValue+"&checkName="+checkName,
	    btn: ['确定', '关闭'],
