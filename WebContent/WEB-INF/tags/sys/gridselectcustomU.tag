...
<%@ attribute name="fieldTypes" type="java.lang.String" required="true" description="字典属性值"%>
<%@ attribute name="filter" type="java.lang.String" required="true" description="过滤属性值"%>
...
<%@ attribute name="callbackKey" type="java.lang.String" required="false" description="回掉函数Key"%>
...
function searchGrid${id}(){
	top.layer.open({
	    ...
	    title:"${title}",
	    name:'friend',
	    content: "${url}?fieldLabels=${fieldLabels}&fieldKeys=${fieldKeys}&fieldTypes=${fieldTypes}&filter=${filter}&url=${url}&searchLabel=${searchLabel}&searchKey=${searchKey}" ,
	    btn: ['确定', '关闭'],
	    yes: function(index, layero){
	    	 var iframeWin = layero.find('iframe')[0].contentWindow; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
	    	 var objstr = iframeWin.getSelectedItem();
	    	 if(!objstr){
	    	  return;
	    	 } 
	    	
	    	 /* 下面是要赋值的语句，需要对应和数组的关系，并且赋值 */
	    
         	 $("#${id}Id").val(objstr[0]);
	    	 $("#${id}Name").val(objstr[1]); 
			 getGridSelectItem(objstr);
	    	/*  afterGridSelect('${callbackKey}',item); */
