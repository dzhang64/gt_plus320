...
<%@ attribute name="showFullPathName" type="java.lang.String" required="false" description="是否显示全路径名称"%>
...
<script type="text/javascript">
	var showFullPathName = ("${showFullPathName}" != "");
	$("#${id}Button, #${id}Name").click(function(){
		...
		var theUrl = "${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}" + "&selectIds=${value}";
		    ...
		    btn: ['确定', '取消']
    	       ,yes: function(index, layero){ //或者使用btn1
						...
						for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
							...
							ids.push(nodes[i].id);
							if(showFullPathName) { 
								names.push(nodes[i].fullPathName);
							} else {
