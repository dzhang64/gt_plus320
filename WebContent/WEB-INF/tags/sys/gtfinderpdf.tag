	...
	if($("#${input}").val()!=null&&$("#${input}").val()!=""){
		...
		for(var i = 0; i < data.length; i++){
			$("#${input}FileListDiv").html(
				$("#${input}FileListDiv").html()
				+ "<span id='" + data[i].uuid + "' name='${input}Allow'>"
					+ "<span style='color:#676A6C;'>" + data[i].filename + "</span>"
					+ "&emsp;"
					+ "<span style='color:#AAA;'>" + data[i].fileSize + "</span>"
					+ "&emsp;"
					+ "<a onclick='${input}View(\"" + data[i].pdfUrl + "\"&#44;\"" + data[i].fileUrl + "\")'>浏览</a>&nbsp;"
					+ "<a href='" + data[i].fileUrl + "'>下载</a>"
					+ "&nbsp;"
					+ "<a onclick='${input}Remove(\"" + data[i].uuid + "\");' class='set'>删除</a>"
					+ "<br>"
				+ "</span>"
			);
		}
...
function ${input}AjaxUpload(formData,filename){
	$.ajax({
		type:"post",
		url:"${ctx}${uploadPath}/fileUploadPdf",
		data:formData,
		processData : false,     
  		contentType : false, 
		dataType:"json",
			...
			if(data=="ExceedSize"){
				...
				$("#${input}FileListDiv").html(
						...
						+ "<span id='" + data.uuid + "' name='${input}Allow'>"
							...
							+ "&emsp;"
							+ "<a onclick='${input}View(\"" + data.pdfUrl + "\"&#44;\"" + data.fileUrl + "\")'>浏览</a>&nbsp;"
...
function ${input}View(pdfUrl,fileUrl){
	if (pdfUrl != '') {
		window.open('${ctxStatic}/pdfjs/web/viewer.html?file=' + pdfUrl);
	} else {
		window.open(fileUrl);
	}
}
