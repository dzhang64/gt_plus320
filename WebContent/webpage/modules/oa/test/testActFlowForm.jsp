...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/oa/test/testActFlow/save",$('#inputForm').serialize(),function(data){
						if(data.success){
		...
		function actInit() {
			if (jp.checkTaskPermission("${testActFlow.procTaskPermission}")) {
		...
		function ruleArgsInit(){
			if ($('#id').val() != null && $('#id').val().trim() != ''){
				var ruleArgs = eval('('+ '${testActFlow.ruleArgsJson}' +')');
...
<body class="hideScroll">
		<form:form id="inputForm" modelAttribute="testActFlow" class="form-horizontal">
		<form:hidden path="id"/>
		
		<input type="hidden" id="act.taskId" name="act.taskId" value="${testActFlow.act.taskId }"/>
		<input type="hidden" id="act.taskName" name="act.taskName" value="${testActFlow.act.taskName }"/>
		<input type="hidden" id="act.taskDefKey" name="act.taskDefKey" value="${testActFlow.act.taskDefKey }"/>
		<input type="hidden" id="procInsId" name="act.procInsId" value="${testActFlow.act.procInsId }"/>
		<input type="hidden" id="act.procDefId" name="act.procDefId" value="${testActFlow.act.procDefId }"/>
		<input type="hidden" id="flag" name="act.flag"/>
		<input type="hidden" id="tempLoginName" name="tempLoginName"/>
		<div id="tempLoginNameDiv"></div>
		
		<input type="hidden" name="dirtyUpdateDate" value="${testActFlow.updateDate}" />
		<form:hidden path="viewFlag" />
		<sys:message content="${message}"/>	
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right">材料名称：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="goods" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="nums" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">采购人：</label></td>
					...
					<td class="width-15 active"><label class="pull-right">采购日期：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<input id="buyDate" name="buyDate" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${testActFlow.buyDate}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
				</tr>
				...
				<c:if test="${not empty testActFlow.id}">
				<tr>
					<td class="width-15 active"><label class="pull-right">流程日志：</label></td>
					<td class="width-35" colspan="3">
							<a class="btn btn-white btn-sm " href="javascript:openDialog('查看流程历史','${ctx}/act/task/trace/photo/${testActFlow.act.procDefId}/${testActFlow.procInsId}','1000px', '600px')">流程图</a>
							<div style="margin-bottom:5px;"></div>
							<act:histoicFlowCustom procInsId="${testActFlow.act.procInsId}" />
					</td>
				</tr>
				</c:if>
		 	</tbody>
