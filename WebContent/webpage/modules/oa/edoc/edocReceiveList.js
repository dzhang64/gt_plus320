...
$(document).ready(function() {
	$('#edocReceiveTable').bootstrapTable({
		...
		url: "${ctx}/oa/edoc/edocReceive/data/${path==null?'Todo':path}", //这个接口需要处理bootstrap table传递的固定参数,并返回特定格式的json数据
		...
		columns: [
			...
			,{
				...
				,formatter:function(value, row , index){
					return jp.getDictLabel(${fns:toJson(fns:getDictList('oa_edoc_type'))}, value, "");
				}
			}
			,{
				field: 'innerNumber',
			...
			,{
				...
				,formatter:function(value, row , index){
					return jp.getDictLabel(${fns:toJson(fns:getDictList('oa_edoc_dense'))}, value, "-");
				}
			}
			,{
				field: 'urgency',
