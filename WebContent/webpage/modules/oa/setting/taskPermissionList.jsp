...
<head>
	...
	<%@ include file="taskPermissionList.js" %>
...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="taskPermission" action="${ctx}/oa/setting/taskPermission/" method="post" class="form-inline">
		...
		<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
			...
			<span>权限类型：</span>
				<form:select path="type"  class="form-control m-b">
					...
					<form:options items="${fns:getDictList('oa_task_permission_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
			<div class="form-group">
			<span>权限类别：</span>
				<form:select path="category"  class="form-control m-b">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('oa_task_permission_category')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</div>
			<div class="form-group">
			<span>是否启用：</span>
	...
	<!-- 工具栏 -->
	<div id="toolbar">
		<shiro:hasPermission name="oa:setting:taskPermission:add">
		...
		<shiro:hasPermission name="oa:setting:taskPermission:import">
		</shiro:hasPermission>
		
	</div>

	<!-- 表格 -->
	<table id="taskPermissionTable"   data-toolbar="#toolbar" data-id-field="id"></table>
