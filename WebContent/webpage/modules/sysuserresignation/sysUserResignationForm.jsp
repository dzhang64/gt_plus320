...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/sysuserresignation/sysUserResignation/save",$('#inputForm').serialize(),function(data){
						if(data.success){
		...
		/*function actInit() {
			if (jp.checkTaskPermission("${sysUserResignation.procTaskPermission}")) {
		...
		function ruleArgsInit(){
				var ruleArgs = eval('('+ '${sysUserResignation.ruleArgsJson}' +')');
       ...
       function ruleArgsUser() {
		...
		if (jp.isNeedUserList(param.type)) {
			...
			$.ajax({
						type : 'post',
						url:'${ctx}/sysuserresignation/sysUserResignation/getUserList',
						data : data,
						dataType : 'json',
...
<body class="hideScroll">
		...
		<form:form id="inputForm" modelAttribute="sysUserResignation" class="form-horizontal">
		<form:hidden path="id"/>
		
		<input type="hidden" id="act.taskId" name="act.taskId" value="${sysUserResignation.act.taskId }"/>
		<input type="hidden" id="act.taskName" name="act.taskName" value="${sysUserResignation.act.taskName }"/>
		<input type="hidden" id="act.taskDefKey" name="act.taskDefKey" value="${sysUserResignation.act.taskDefKey }"/>
		<input type="hidden" id="procInsId" name="act.procInsId" value="${sysUserResignation.act.procInsId }"/>
		<input type="hidden" id="act.procDefId" name="act.procDefId" value="${sysUserResignation.act.procDefId }"/>
		...
		<input type="hidden" name="dirtyUpdateDate" value="${sysUserResignation.updateDate}" />
		<form:hidden path="viewFlag" />
		<sys:message content="${message}"/>	
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right">离职人员：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<sys:gridselectcustomU url="${ctx}/userinfo/userInfo/selectUserInfo" id="name" name="name.id"  value="${sysUserResignation.name.id}"  title="选择离职人员" labelName="name.name" 
						 labelValue="${sysUserResignation.name.name}" cssClass="form-control required" fieldLabels="姓名|工号|部门|区域|员工状态|员工类型" fieldKeys="name|nameCode|office|area|userStatus|userType" fieldTypes="0|0|0|0|sys_user_status|sys_user_type" filter="" searchLabel="姓名" searchKey="name"></sys:gridselectcustomU>
						</fieldset>
					</td>
				...
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set" colspan="3">
						<input id="leaveDate" name="leaveDate" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserResignation.leaveDate}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
				<tr>
					<td class="width-15 active"><label class="pull-right">离职原因：</label></td>
			
					<td class="width-35" colspan="3">
						<fieldset class="set">
						<form:textarea path="leaveReason" htmlEscape="false" rows="4"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
					<td class="width-15 active"><label class="pull-right">离职类型：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:select path="leaveType" class="form-control ">
							<form:option value="" label=""/>
							<form:options items="${fns:getDictListEdit('leave_type',sysUserResignation.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">交接设备：</label></td>
			
					<td class="width-35" colspan="3">
						<fieldset class="set">
						<form:textarea path="transferEquipment" htmlEscape="false" rows="4"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">未交接耗材：</label></td>
			
					<td class="width-35" colspan="3">
						<fieldset class="set">
						<form:textarea path="noTransfer" htmlEscape="false" rows="4"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">认证扣款：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="certificationDeduction" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">未付工资金额：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="noSalaryPay" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">未报销金额：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="noReimbursement" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="loanAmount" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">未付金额合计：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="amountSum" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">离职结算：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="leaveSum" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				...
				<c:if test="${not empty sysUserResignation.id}">
				<tr>
					...
					<td class="width-35" colspan="3">
						...
						<a class="btn btn-white btn-sm " href="javascript:openDialog('查看流程历史','${ctx}/act/task/trace/photo/${sysUserResignation.act.procDefId}/${sysUserResignation.procInsId}','1000px', '600px')">流程图</a>
						<div style="margin-bottom:5px;"></div>
						<act:histoicFlowCustom procInsId="${sysUserResignation.act.procInsId}" />
					</td>
				</tr>
				</c:if>
		 	</tbody>
