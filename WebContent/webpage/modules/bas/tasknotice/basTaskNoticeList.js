...
$(document).ready(function() {
	$('#basTaskNoticeTable').bootstrapTable({
		...
		url: "${ctx}/bas/tasknotice/basTaskNotice/data", //这个接口需要处理bootstrap table传递的固定参数,并返回特定格式的json数据
		...
		columns: [
			...
			,{
				...
				formatter:function(value, row , index){
					return jp.getDictLabel(${fns:toJson(fns:getDictList('oa_notify_read'))}, value, "-");
				}
			}
			,{ //操作列
				field: 'operate',
				...
				formatter: function operateFormatter(value, row, index) {
					...
					return [
						...
						"<a href='javascript:jp.viewTask(\"${ctx}" + row.remarks + "\",\"" + row.content + "\")' class='btn btn-info btn-xs'><i class='fa fa-search-plus'></i> 查看任务</a>",
						</shiro:hasPermission>
...
function edit(id,isView){
	...
	if (isView) {
		jp.openDialogView('查看任务通知', "${ctx}/bas/tasknotice/basTaskNotice/form?viewFlag=set_audit&id=" + id,'800px', '500px');
	} else {
	<shiro:hasPermission name="bas:tasknotice:basTaskNotice:edit">
		jp.openDialog('编辑任务通知', "${ctx}/bas/tasknotice/basTaskNotice/form?id=" + id,'800px', '500px', $('#basTaskNoticeTable'));
	</shiro:hasPermission>
	<shiro:lacksPermission name="bas:tasknotice:basTaskNotice:edit">
		jp.openDialogView('查看任务通知', "${ctx}/bas/tasknotice/basTaskNotice/form?viewFlag=set_audit&id=" + id,'800px', '500px');
