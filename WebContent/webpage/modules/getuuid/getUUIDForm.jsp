...
<head>
	...
	<script type="text/javascript">
		...
		function doSubmit(table, index,pValue){
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/getuuid/getUUID/save",$('#inputForm').serialize(),function(data){
						if(data.success){
...
<body class="hideScroll">
		...
		<form:form id="inputForm" modelAttribute="getUUID" class="form-horizontal">
		<form:hidden path="id"/>
		
		
		<input type="hidden" name="dirtyUpdateDate" value="${getUUID.updateDate}" />
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					...
					<td class="width-15 active"><label class="pull-right">UUID数量：</label></td>
