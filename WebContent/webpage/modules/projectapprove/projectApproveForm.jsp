...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/projectapprove/projectApprove/save",$('#inputForm').serialize(),function(data){
						if(data.success){
		...
		/*function actInit() {
			if (jp.checkTaskPermission("${projectApprove.procTaskPermission}")) {
		...
		function ruleArgsInit(){
				var ruleArgs = eval('('+ '${projectApprove.ruleArgsJson}' +')');
       ...
       function ruleArgsUser() {
		...
		if (jp.isNeedUserList(param.type)) {
			...
			$.ajax({
						type : 'post',
						url:'${ctx}/projectapprove/projectApprove/getUserList',
						data : data,
						dataType : 'json',
...
<body class="hideScroll">
		...
		<form:form id="inputForm" modelAttribute="projectApprove" class="form-horizontal">
		<form:hidden path="id"/>
		
		<input type="hidden" id="act.taskId" name="act.taskId" value="${projectApprove.act.taskId }"/>
		<input type="hidden" id="act.taskName" name="act.taskName" value="${projectApprove.act.taskName }"/>
		<input type="hidden" id="act.taskDefKey" name="act.taskDefKey" value="${projectApprove.act.taskDefKey }"/>
		<input type="hidden" id="procInsId" name="act.procInsId" value="${projectApprove.act.procInsId }"/>
		<input type="hidden" id="act.procDefId" name="act.procDefId" value="${projectApprove.act.procDefId }"/>
		...
		<input type="hidden" name="dirtyUpdateDate" value="${projectApprove.updateDate}" />
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="projectNum" htmlEscape="false"    class="form-control "  disabled="true" style="background-color:#F8F8F8;"/>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<form:select path="projectAttribute" class="form-control ">
							...
							<form:options items="${fns:getDictListEdit('project_attribute',projectApprove.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						...
						<form:select path="projectType" class="form-control ">
							<form:option value="" label=""/>
							<form:options items="${fns:getDictListEdit('project_type',projectApprove.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<form:select path="productType" class="form-control ">
							<form:option value="" label=""/>
							<form:options items="${fns:getDictListEdit('product',projectApprove.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<sys:treeselect id="area" name="area.id" value="${projectApprove.area.id}" labelName="area.name" labelValue="${projectApprove.area.name}"
				...
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<sys:treeselect id="office" name="office.id" value="${projectApprove.office.id}" labelName="office.name" labelValue="${projectApprove.office.name}"
			        ...
			        <td class="width-35">
						...
						<form:select path="deviceCompany" class="form-control ">
							<form:option value="" label=""/>
							<form:options items="${fns:getDictListEdit('device_company_type',projectApprove.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="cooperative" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						...
						<form:select path="operator" class="form-control ">
							<form:option value="" label=""/>
							<form:options items="${fns:getDictListEdit('operator',projectApprove.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<input id="startTime" name="startTime" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${projectApprove.startTime}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<input id="endTime" name="endTime" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${projectApprove.endTime}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<sys:gridselectcustom url="${ctx}/userinfo/userInfo/selectUserInfo" id="projectmanager" name="projectmanager.id"  value="${projectApprove.projectmanager.id}"  title="选择项目经理" labelName="projectmanager.name" 
						 labelValue="${projectApprove.projectmanager.name}" cssClass="form-control required" fieldLabels="姓名|工号|部门|区域|员工状态|员工类型" fieldKeys="name|nameCode|office|area|userStatus|userType" fieldTypes="0|0|0|0|sys_user_status|sys_user_type" filter="" searchLabel="姓名" searchKey="name"></sys:gridselectcustom>
						</fieldset>
					</td>
				</tr>
				<tr>
					...
					<td class="width-35" colspan="3">
						...
						<input type="hidden" id="attachfile" name="attachfile" value='${projectApprove.attachfile}' />
						<sys:gtfinder input="attachfile" uploadPath="/projectapprove/projectApprove"></sys:gtfinder>
					</td>
				</tr>
				<tr>
				...
				<c:if test="${not empty projectApprove.id}">
				<tr>
					...
					<td class="width-35" colspan="3">
						...
						<a class="btn btn-white btn-sm " href="javascript:openDialog('查看流程历史','${ctx}/act/task/trace/photo/${projectApprove.act.procDefId}/${projectApprove.procInsId}','1000px', '600px')">流程图</a>
						<div style="margin-bottom:5px;"></div>
						<act:histoicFlowCustom procInsId="${projectApprove.act.procInsId}" />
					</td>
				</tr>
				</c:if>
		 	</tbody>
