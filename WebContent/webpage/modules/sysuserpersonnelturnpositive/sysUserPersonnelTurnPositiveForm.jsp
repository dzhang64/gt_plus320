...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/sysuserpersonnelturnpositive/sysUserPersonnelTurnPositive/save",$('#inputForm').serialize(),function(data){
						if(data.success){
		...
		/*function actInit() {
			if (jp.checkTaskPermission("${sysUserPersonnelTurnPositive.procTaskPermission}")) {
		...
		function ruleArgsInit(){
				var ruleArgs = eval('('+ '${sysUserPersonnelTurnPositive.ruleArgsJson}' +')');
				if (ruleArgs != null && ruleArgs.form !=null) {
					$.each(ruleArgs.form, function(key, value){
						...
						$('#'+key).parent().parent().parent().attr('style', 'display: none;');
						}
						//==4 向上1级显示
						else if (value == 4) {
						...
						//==5 向上1级显示&可编辑
							...
							$('#'+key).parent().removeAttr('disabled');
							$('#'+key).parent().parent().removeAttr('disabled');
       ...
       function ruleArgsUser() {
		...
		if (jp.isNeedUserList(param.type)) {
			...
			$.ajax({
						type : 'post',
						url:'${ctx}/sysuserpersonnelturnpositive/sysUserPersonnelTurnPositive/getUserList',
						data : data,
						dataType : 'json',
...
<body class="hideScroll">
		...
		<form:form id="inputForm" modelAttribute="sysUserPersonnelTurnPositive" class="form-horizontal">
		<form:hidden path="id"/>
		
		<input type="hidden" id="act.taskId" name="act.taskId" value="${sysUserPersonnelTurnPositive.act.taskId }"/>
		<input type="hidden" id="act.taskName" name="act.taskName" value="${sysUserPersonnelTurnPositive.act.taskName }"/>
		<input type="hidden" id="act.taskDefKey" name="act.taskDefKey" value="${sysUserPersonnelTurnPositive.act.taskDefKey }"/>
		<input type="hidden" id="procInsId" name="act.procInsId" value="${sysUserPersonnelTurnPositive.act.procInsId }"/>
		<input type="hidden" id="act.procDefId" name="act.procDefId" value="${sysUserPersonnelTurnPositive.act.procDefId }"/>
		...
		<input type="hidden" name="dirtyUpdateDate" value="${sysUserPersonnelTurnPositive.updateDate}" />
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<sys:gridselectcustom url="${ctx}/userinfo/userInfo/selectUserInfo" id="name" name="name.id"  value="${sysUserPersonnelTurnPositive.name.id}"  title="选择姓名" labelName="name.name" 
						 labelValue="${sysUserPersonnelTurnPositive.name.name}" cssClass="form-control required" fieldLabels="姓名|工号|部门|区域|员工状态|员工类型" fieldKeys="name|nameCode|office|area|userStatus|userType" fieldTypes="0|0|0|0|sys_user_status|sys_user_type" filter="3" searchLabel="姓名" searchKey="name"></sys:gridselectcustom>
						</fieldset>
						
					</td>
				<tr>
					<td class="width-15 active"><label class="pull-right">试用期考核表：</label></td>
			
					<td class="width-35" colspan="3">
						
						<input type="hidden" id="assessmentForm" name="assessmentForm" value='${sysUserPersonnelTurnPositive.assessmentForm}' />
						<sys:gtfinderW input="assessmentForm" uploadPath="/sysuserinterentry/sysInternUserInformation"></sys:gtfinderW>
					</td>
				</tr>
					<td class="width-15 active"><label class="pull-right">转正前薪资级别：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<sys:gridselectcustom url="${ctx}/salarylevel/salaryLevel/selectsalary" id="beforeSalary" name="beforeSalary.id"  value="${sysUserPersonnelTurnPositive.beforeSalary.id}"  title="选择转正前薪资级别" labelName="beforeSalary.level" 
						 labelValue="${sysUserPersonnelTurnPositive.beforeSalary.level}" cssClass="form-control required" fieldLabels="等级|基本工资|绩效工资|补贴" fieldKeys="level|basicWage|performancePay|subsidy"  fieldTypes="0|0|0|0" filter=""  searchLabel="等级" searchKey="level" ></sys:gridselectcustom>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">转正前等级工资：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="gradePayBefore" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">转正后薪资级别：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<sys:gridselectcustom url="${ctx}/salarylevel/salaryLevel/selectsalary" id="afterSalary" name="afterSalary.id"  value="${sysUserPersonnelTurnPositive.afterSalary.id}"  title="选择转正后薪资级别" labelName="afterSalary.level" 
						 labelValue="${sysUserPersonnelTurnPositive.afterSalary.level}" cssClass="form-control " fieldLabels="等级|基本工资|绩效工资|补贴" fieldKeys="level|basicWage|performancePay|subsidy"  fieldTypes="0|0|0|0" filter=""  searchLabel="等级" searchKey="level" ></sys:gridselectcustom>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">转正后等级工资：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="afterGradePay" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<input id="entryTime" name="entryTime" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserPersonnelTurnPositive.entryTime}" pattern="yyyy-MM-dd HH:mm:ss" />"/>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">转正时间：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<input id="turnTime" name="turnTime" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserPersonnelTurnPositive.turnTime}" pattern="yyyy-MM-dd HH:mm:ss" />"/>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<form:select path="userType" class="form-control ">
							...
							<form:options items="${fns:getDictListEdit('sys_user_type',sysUserPersonnelTurnPositive.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
				</tr>
				...
				<c:if test="${not empty sysUserPersonnelTurnPositive.id}">
				<tr>
					...
					<td class="width-35" colspan="3">
						...
						<a class="btn btn-white btn-sm " href="javascript:openDialog('查看流程历史','${ctx}/act/task/trace/photo/${sysUserPersonnelTurnPositive.act.procDefId}/${sysUserPersonnelTurnPositive.procInsId}','1000px', '600px')">流程图</a>
						<div style="margin-bottom:5px;"></div>
						<act:histoicFlowCustom procInsId="${sysUserPersonnelTurnPositive.act.procInsId}" />
					</td>
				</tr>
				</c:if>
		 	</tbody>
