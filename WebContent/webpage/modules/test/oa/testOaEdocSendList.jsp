...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="testOaEdoc" action="${ctx}/test/oa/testOaEdoc/send" method="post" class="form-inline">
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<tbody>
			<c:forEach items="${page.list}" var="act">
				...
				<tr>
					<td>
						<%-- <a href="${ctx}/act/task/form?procInsId=${hisProcIns.id}&procDefId=${procDef.id}&status=${status}">${fns:abbr(not empty vars.map.title ? vars.map.title : task.id, 60)}</a>
						 --%>
					...
					<td>
						<a  href="javascript:openDialog('跟踪','${ctx}/act/task/trace/photo/${histTask.processDefinitionId}/${task.executionId}','1000px', '600px')">${task.name}</a>
					...
					<td>${procDef.name}</td>
					<td>${act.vars.map.applyUser}</td>
					<td><fmt:formatDate value="${task.endTime}" type="both"/></td>
					<td><b title='流程版本号'>V : ${procDef.version}</b></td>
					<td>
						<%-- <a href="${ctx}/act/task/form?procInsId=${hisProcIns.id}&procDefId=${procDef.id}&status=${status}">详情</a> --%>
					<a href="${ctx}/act/task/form?taskId=${task.id}&taskName=${fns:urlEncode(task.name)}&taskDefKey=${task.taskDefinitionKey}&procInsId=${task.processInstanceId}&procDefId=${task.processDefinitionId}&status=${status}">${fns:abbr(not empty vars.map.title ? vars.map.title : task.id, 60)}</a>
					</td>
				</tr>
			</c:forEach>
