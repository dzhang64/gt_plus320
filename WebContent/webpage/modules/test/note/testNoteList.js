...
$(document).ready(function() {
	$('#testNoteTable').bootstrapTable({
		...
		url: "${ctx}/test/note/testNote/data", //这个接口需要处理bootstrap table传递的固定参数,并返回特定格式的json数据
...
//批量删除
function deleteAll(){
	jp.confirm('确认要删除选中富文本测试记录吗？', function(){
		//jp.loading();  	
...
//添加   
function add(){
	jp.openDialog('添加富文本测试', "${ctx}/test/note/testNote/form",'800px', '500px', $('#testNoteTable'));
}
...
function edit(id,isView){
	...
	if (isView) {
		jp.openDialogView('查看富文本测试', "${ctx}/test/note/testNote/form?id=" + id,'800px', '500px');
	} else {
	<shiro:hasPermission name="test:note:testNote:edit">
		jp.openDialog('编辑富文本测试', "${ctx}/test/note/testNote/form?id=" + id,'800px', '500px', $('#testNoteTable'));
	</shiro:hasPermission>
	<shiro:lacksPermission name="test:note:testNote:edit">
		jp.openDialogView('查看富文本测试', "${ctx}/test/note/testNote/form?id=" + id,'800px', '500px');
...
//单条删除
function del(id){
	jp.confirm('确认要删除该富文本测试吗？',  function(){
		jp.get("${ctx}/test/note/testNote/delete?id=" + id, function(data){
			if(data.success){
				$('#testNoteTable').bootstrapTable('refresh');
				jp.success(data.msg);
			}else{
