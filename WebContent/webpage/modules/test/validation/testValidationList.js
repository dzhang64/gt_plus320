...
$(document).ready(function() {
	$('#testValidationTable').bootstrapTable({
		...
		url: "${ctx}/test/validation/testValidation/data", //这个接口需要处理bootstrap table传递的固定参数,并返回特定格式的json数据
		...
		onContextMenuItem: function(row, $el){
			if($el.data("item") == "edit"){
				...
				jp.confirm('确认要删除该测试校验功能记录吗？', function(){
					jp.loading();
			...
			,{ //操作列
				...
				events: {
					'click .view': function (e, value, row, index) {
						jp.openDialogView('查看测试校验功能', '${ctx}/test/validation/testValidation/form?id=' + row.id,'800px', '500px');
					},
					'click .edit': function (e, value, row, index) {
						var windowName = $(".edit").attr("dealname")+'测试校验功能';
						var btns = $(".edit").attr("dealbtn");
						var params = $(".edit").attr("dealdata");
						jp.openDialog(windowName, '${ctx}/test/validation/testValidation/form?id=' + row.id,'800px', '500px', $('#testValidationTable'), btns, params);
					},
					'click .edit2': function (e, value, row, index) {
						var windowName = $(".edit2").attr("dealname")+'测试校验功能';
						var btns = $(".edit2").attr("dealbtn");
						var params = $(".edit2").attr("dealdata");
						jp.openDialog(windowName, '${ctx}/test/validation/testValidation/form?id=' + row.id,'800px', '500px', $('#testValidationTable'), btns, params);
					},
					'click .edit3': function (e, value, row, index) {
						var windowName = $(".edit3").attr("dealname")+'测试校验功能';
						var btns = $(".edit3").attr("dealbtn");
						var params = $(".edit3").attr("dealdata");
						jp.openDialog(windowName, '${ctx}/test/validation/testValidation/form?id=' + row.id,'800px', '500px', $('#testValidationTable'), btns, params);
					},
					'click .edit4': function (e, value, row, index) {
						var windowName = $(".edit4").attr("dealname")+'测试校验功能';
						var btns = $(".edit4").attr("dealbtn");
						var params = $(".edit4").attr("dealdata");
						jp.openDialog(windowName, '${ctx}/test/validation/testValidation/form?id=' + row.id,'800px', '500px', $('#testValidationTable'), btns, params);
					},
					'click .edit5': function (e, value, row, index) {
						var windowName = $(".edit5").attr("dealname")+'测试校验功能';
...
//批量删除
function deleteAll(){
	jp.confirm('确认要删除选中测试校验功能记录吗？', function(){
		//jp.loading();  	
...
//添加   
function add(){
	jp.openDialog('添加测试校验功能', "${ctx}/test/validation/testValidation/form",'800px', '500px', $('#testValidationTable'));
}
...
function edit(id,isView){
	...
	if (isView) {
		jp.openDialogView('查看测试校验功能', "${ctx}/test/validation/testValidation/form?id=" + id,'800px', '500px');
	} else {
	<shiro:hasPermission name="test:validation:testValidation:edit">
		jp.openDialog('编辑测试校验功能', "${ctx}/test/validation/testValidation/form?id=" + id,'800px', '500px', $('#testValidationTable'));
	</shiro:hasPermission>
	<shiro:lacksPermission name="test:validation:testValidation:edit">
		jp.openDialogView('查看测试校验功能', "${ctx}/test/validation/testValidation/form?id=" + id,'800px', '500px');
...
//单条删除
function del(id){
	jp.confirm('确认要删除该测试校验功能吗？',  function(){
		jp.get("${ctx}/test/validation/testValidation/delete?id=" + id, function(data){
			if(data.success){
				$('#testValidationTable').bootstrapTable('refresh');
				jp.success(data.msg);
			}else{
