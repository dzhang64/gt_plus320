...
<body class="gray-bg">
	...
	<form:form id="searchForm" modelAttribute="goods" action="${ctx}/test/grid/goods/" method="post" class="form-inline">
		...
		<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
			...
			<div class="form-group">
			<span>所属类型：</span>
				<sys:gridselectcustom url="${ctx}/test/grid/goods/selectcategory" id="category" name="category"  value="${goods.category.id}"  title="选择所属类型" labelName="category.name" 
					labelValue="${goods.category.name}" cssClass="form-control input-sm required" fieldLabels="商品分类|备注" fieldKeys="name|remarks" searchLabel="分类名" searchKey="name" callbackKey="category"></sys:gridselectcustom>
			</div>
	...
	<div id="toolbar">
		...
		<shiro:hasPermission name="test:grid:goods:import">
			...
			<div id="importBox" class="hide">
				<form id="importForm" action="${ctx}/test/grid/goods/import" method="post" enctype="multipart/form-data"
	...
	<!-- 表格 -->
	<table id="goodsTable"   data-toolbar="#toolbar" data-id-field="id"></table>
