...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/sysusercomputersubsidiesapply/sysUserComputerSubsidiesApply/save",$('#inputForm').serialize(),function(data){
						if(data.success){
		...
		/*function actInit() {
			if (jp.checkTaskPermission("${sysUserComputerSubsidiesApply.procTaskPermission}")) {
		...
		function ruleArgsInit(){
				var ruleArgs = eval('('+ '${sysUserComputerSubsidiesApply.ruleArgsJson}' +')');
       ...
       function ruleArgsUser() {
		...
		if (jp.isNeedUserList(param.type)) {
			...
			$.ajax({
						type : 'post',
						url:'${ctx}/sysusercomputersubsidiesapply/sysUserComputerSubsidiesApply/getUserList',
						data : data,
						dataType : 'json',
...
<body class="hideScroll">
		...
		<form:form id="inputForm" modelAttribute="sysUserComputerSubsidiesApply" class="form-horizontal">
		<form:hidden path="id"/>
		
		<input type="hidden" id="act.taskId" name="act.taskId" value="${sysUserComputerSubsidiesApply.act.taskId }"/>
		<input type="hidden" id="act.taskName" name="act.taskName" value="${sysUserComputerSubsidiesApply.act.taskName }"/>
		<input type="hidden" id="act.taskDefKey" name="act.taskDefKey" value="${sysUserComputerSubsidiesApply.act.taskDefKey }"/>
		<input type="hidden" id="procInsId" name="act.procInsId" value="${sysUserComputerSubsidiesApply.act.procInsId }"/>
		<input type="hidden" id="act.procDefId" name="act.procDefId" value="${sysUserComputerSubsidiesApply.act.procDefId }"/>
		...
		<input type="hidden" name="dirtyUpdateDate" value="${sysUserComputerSubsidiesApply.updateDate}" />
		<form:hidden path="viewFlag" />
		<sys:message content="${message}"/>	
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right">电脑配置：</label></td>
			
					<td class="width-35" colspan="3">
						<fieldset class="set">
						<form:textarea path="computerConfiguration" htmlEscape="false" rows="4"    class="form-control "/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">电脑序列号：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="computerSerialNumber" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">购买日期：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<input id="buyDate" name="buyDate" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserComputerSubsidiesApply.buyDate}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">起租日期：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<input id="beginDate" name="beginDate" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserComputerSubsidiesApply.beginDate}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
					<td class="width-15 active"><label class="pull-right">终租日期：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<input id="endDate" name="endDate" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserComputerSubsidiesApply.endDate}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">补贴标准：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<form:input path="subsidyStandard" htmlEscape="false"    class="form-control "/>
						</fieldset>
					</td>
				...
				<c:if test="${not empty sysUserComputerSubsidiesApply.id}">
				<tr>
					...
					<td class="width-35" colspan="3">
						...
						<a class="btn btn-white btn-sm " href="javascript:openDialog('查看流程历史','${ctx}/act/task/trace/photo/${sysUserComputerSubsidiesApply.act.procDefId}/${sysUserComputerSubsidiesApply.procInsId}','1000px', '600px')">流程图</a>
						<div style="margin-bottom:5px;"></div>
						<act:histoicFlowCustom procInsId="${sysUserComputerSubsidiesApply.act.procInsId}" />
					</td>
				</tr>
				</c:if>
		 	</tbody>
