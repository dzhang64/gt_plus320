...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() { 
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					jp.post("${ctx}/sysuserinterntranscription/sysUserInternTranscription/save",$('#inputForm').serialize(),function(data){
						if(data.success){
		...
		/*function actInit() {
			if (jp.checkTaskPermission("${sysUserInternTranscription.procTaskPermission}")) {
		...
		function ruleArgsInit(){
				var ruleArgs = eval('('+ '${sysUserInternTranscription.ruleArgsJson}' +')');
       ...
       function ruleArgsUser() {
		...
		if (jp.isNeedUserList(param.type)) {
			...
			$.ajax({
						type : 'post',
						url:'${ctx}/sysuserinterntranscription/sysUserInternTranscription/getUserList',
						data : data,
						dataType : 'json',
...
<body class="hideScroll">
		...
		<form:form id="inputForm" modelAttribute="sysUserInternTranscription" class="form-horizontal">
		<form:hidden path="id"/>
		
		<input type="hidden" id="act.taskId" name="act.taskId" value="${sysUserInternTranscription.act.taskId }"/>
		<input type="hidden" id="act.taskName" name="act.taskName" value="${sysUserInternTranscription.act.taskName }"/>
		<input type="hidden" id="act.taskDefKey" name="act.taskDefKey" value="${sysUserInternTranscription.act.taskDefKey }"/>
		<input type="hidden" id="procInsId" name="act.procInsId" value="${sysUserInternTranscription.act.procInsId }"/>
		<input type="hidden" id="act.procDefId" name="act.procDefId" value="${sysUserInternTranscription.act.procDefId }"/>
		...
		<input type="hidden" name="dirtyUpdateDate" value="${sysUserInternTranscription.updateDate}" />
		<form:hidden path="viewFlag" />
		<sys:message content="${message}"/>	
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				<tr>
					<td class="width-15 active"><label class="pull-right">实习人员：</label></td>
			
			        <td class="width-35" colspan="3">
						<fieldset class="set">
						<sys:gridselectcustom url="${ctx}/userinfo/userInfo/selectUserInfo" id="name" name="name.id"  value="${sysUserInternTranscription.name.id}"  title="选择实习人员" labelName="name.name" 
						 labelValue="${sysUserInternTranscription.name.name}" cssClass="form-control required" fieldLabels="姓名|工号|部门|区域|员工状态|员工类型" fieldKeys="name|nameCode|office|area|userStatus|userType" fieldTypes="0|0|0|0|sys_user_status|sys_user_type" filter="1" searchLabel="姓名" searchKey="name"></sys:gridselectcustom>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<input id="startTime" name="startTime" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserInternTranscription.startTime}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						...
						<input id="endTime" name="endTime" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserInternTranscription.endTime}" pattern="yyyy-MM-dd" />"/>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						<fieldset class="set">
						<sys:treeselect id="ssc" name="ssc.id" value="${sysUserInternTranscription.ssc.id}" labelName="ssc.name" labelValue="${sysUserInternTranscription.ssc.name}"
			        ...
			        <td class="width-35">
						...
						<form:select path="userStatus" class="form-control ">
							...
							<form:options items="${fns:getDictListEdit('sys_user_status',sysUserInternTranscription.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
				</tr>
				<tr>
			        ...
			        <td class="width-35">
						...
						<form:select path="userType" class="form-control ">
							<form:option value="" label=""/>
							<form:options items="${fns:getDictListEdit('sys_user_type',sysUserInternTranscription.id)}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
						</fieldset>
					</td>
			        ...
			        <td class="width-35">
						...
						<input id="beginDate" name="beginDate" type="text" maxlength="20" class="laydate-icon form-control layer-date "
							value="<fmt:formatDate value="${sysUserInternTranscription.beginDate}" pattern="yyyy-MM-dd HH:mm:ss" />"/>
						</fieldset>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">工资级别：</label></td>
			
			        <td class="width-35">
						<fieldset class="set">
						<sys:gridselectcustom url="${ctx}/salarylevel/salaryLevel/selectsalary" id="level" name="level.id"  value="${sysUserInternTranscription.level.id}"  title="选择工资级别" labelName="level.level" 
						 labelValue="${sysUserInternTranscription.level.level}" cssClass="form-control required" fieldLabels="等级|基本工资|绩效工资|补贴" fieldKeys="level|basicWage|performancePay|subsidy"  fieldTypes="0|0|0|0" filter=""  searchLabel="等级" searchKey="level" ></sys:gridselectcustom>
						</fieldset>
					</td>
				...
				<tr>
					<td class="width-15 active"><label class="pull-right">录用申请表：</label></td>
			
					<td class="width-35" colspan="3">
						
						<input type="hidden" id="transcriptionTable" name="transcriptionTable" value='${sysUserInternTranscription.transcriptionTable}' />
						<sys:gtfinder input="transcriptionTable" uploadPath="/sysuserinterntranscription/sysUserInternTranscription"></sys:gtfinder>
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">毕业证扫描件：</label></td>
			
					<td class="width-35" colspan="3">
						
						<input type="hidden" id="gradeCertificate" name="gradeCertificate" value='${sysUserInternTranscription.gradeCertificate}' />
						<sys:gtfinder input="gradeCertificate" uploadPath="/sysuserinterntranscription/sysUserInternTranscription"></sys:gtfinder>
					</td>
				</tr>
				...
				<c:if test="${not empty sysUserInternTranscription.id}">
				<tr>
					...
					<td class="width-35" colspan="3">
						...
						<a class="btn btn-white btn-sm " href="javascript:openDialog('查看流程历史','${ctx}/act/task/trace/photo/${sysUserInternTranscription.act.procDefId}/${sysUserInternTranscription.procInsId}','1000px', '600px')">流程图</a>
						<div style="margin-bottom:5px;"></div>
						<act:histoicFlowCustom procInsId="${sysUserInternTranscription.act.procInsId}" />
					</td>
				</tr>
				</c:if>
		 	</tbody>
